...
func newIntegrationOperationLogService(deps *integrationDeps) *systemService.OperationLogService {
	return systemService.NewOperationLogService(deps.repo, daoSystem.NewOperationLogDao(deps.repo))
}

func newIntegrationRoleService(deps *integrationDeps) *RoleService {
	return NewRoleService(deps.repo, daoAccount.NewRoleDao(deps.repo), deps.cache, newIntegrationOperationLogService(deps))
}

func newIntegrationUserService(deps *integrationDeps) *UserService {
	roleService := newIntegrationRoleService(deps)
	return NewUserService(deps.repo, daoAccount.NewUserDao(deps.repo), deps.cache, roleService, newIntegrationOperationLogService(deps))
}

func newIntegrationMenuService(deps *integrationDeps) *MenuService {
	return NewMenuService(deps.repo, deps.cache, daoAccount.NewMenuDao(deps.repo), newIntegrationOperationLogService(deps))
}

func insertIntegrationRole(t *testing.T, db *gorm.DB, code, name string) *model.SysRole {
	t.Helper()
	role := &model.SysRole{
		Code: code,
		Name: &name,
	...
	if err := db.Create(role).Error; err != nil {
		t.Fatalf("insert integration role: %v", err)
	}
	return role
}

func insertIntegrationMenu(t *testing.T, db *gorm.DB, parentID int32, menuType, name string) *model.SysMenu {
	t.Helper()
	menu := &model.SysMenu{
		ParentID: parentID,
		MenuType: menuType,
	...
	if err := db.Create(menu).Error; err != nil {
		t.Fatalf("insert integration menu: %v", err)
	}
	return menu
}

func insertIntegrationUser(t *testing.T, db *gorm.DB, username, tel string, roleIds ...int32) *model.SysUser {
	t.Helper()
	activeStatus := constant.UserStatusActive
	password, err := xcryption.HashPassword("abc123")
	if err != nil {
	...
	for _, roleID := range roleIds {
		if err := db.Create(&model.SysUserRole{UserID: user.ID, RoleID: roleID}).Error; err != nil {
			t.Fatalf("insert integration user role: %v", err)
		}
	}
	return user
}

func insertIntegrationRoleMenu(t *testing.T, db *gorm.DB, roleID, menuID int32) {
	t.Helper()
	if err := db.Create(&model.SysRoleMenu{RoleID: roleID, MenuID: menuID}).Error; err != nil {
		t.Fatalf("insert integration role menu: %v", err)
	}
}
