...
// GetRoleById 查询角色
func (r *RoleDao) GetRoleById(ctx context.Context, roleId int32) (*model.SysRole, error) {
	if roleId <= 0 {
...
// GetRoleList 角色列表
func (r *RoleDao) GetRoleList(ctx context.Context, cond *RoleListCondition) ([]*model.SysRole, int64, error) {
	m := r.repo.Query().SysRole
...
func (r *RoleDao) GetMenuListByRoleId(ctx context.Context, roleId int32) ([]*model.SysMenu, error) {
	m := r.repo.Query().SysRoleMenu
	menu := r.repo.Query().SysMenu
	menuList := make([]*model.SysMenu, 0, 10)
	err := m.WithContext(ctx).
		Join(menu, m.MenuID.EqCol(menu.ID)).
